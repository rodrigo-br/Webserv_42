	...
	while (!gSignalInterrupted)
	{
		...
		for (size_t i = 0; i < client_socks.size(); ++i)
    	{
			...
			if (FD_ISSET(client_sock, &write_sockets))
			{
				...
				ssize_t bytes_sent = send(client_sock, response.get_response(), response.get_size(), 0);
       			if (bytes_sent == -1) {
				...
				else if (bytes_sent > 0) 
				{
					if (response.has_body()) {
						ssize_t bytes_body_sent = send(client_sock, response.get_body(), response.body_size(), 0);
